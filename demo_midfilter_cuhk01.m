...
if ~exist('filter_all0.mat', 'file')
        ...
        for auc_level = 1:K_auc
            ...
            for i = 1:nfilter
                ...
                if size(X_all, 1) < max_npat
                    model = libsvmtrain(y_all, X_all, '-t 0 -c 30 -b 1 -q');
                    filters_new{tk, auc_level}(i).w = model.SVs'*model.sv_coef;
                    filters_new{tk, auc_level}(i).b = -model.rho;
                else
                    filters_new{tk, auc_level}(i).w = model.SVs'*model.sv_coef*0;
    ...
    % save filters
    save filter_all.mat filters_new;
    
else
    load filter_all0.mat;
end
